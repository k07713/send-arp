...
void usage() {
	printf("syntax: send-arp <interface> <sip> <dip>\n");
	printf("sample: send-arp enp0s5 192.168.0.1 192.168.0.4\n");
}
...
int main(int argc, char* argv[]) {
	...
	for(int i=2; i<argc; i += 2){
		...
		strcpy(macs, mymac);
		send_arp_request(dev, myip, macs, sip, macd);
		smac = macd;

		//attack table
		arp_attack(dev, dip, mymac, sip, smac);
		sleep(1);
	}
}
